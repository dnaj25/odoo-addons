...
class ResPartner(models.Model):
    ...
    def do_partner_print(self, wizard_partner_ids, data):
        ...
        return self.env.ref(
            'community_accounting_followup.action_report_followup').report_action(
            self, data=datas)

    def do_partner_mail(self):
        ctx = self.env.context.copy()
        ctx['followup'] = True
        template = 'community_accounting_followup.email_template_community_accounting_followup_default'
        unknown_mails = 0
        for partner in self:
    ...
    def get_followup_table_html(self):
        ...
        if partner.unreconciled_aml_ids:
            ...
            report = self.env['report.community_accounting_followup.report_followup']
